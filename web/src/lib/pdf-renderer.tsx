...
function FitReportPdf({ analysis, extracted, citations }: FitReportPdfProps) {
    ...
    <Document>
      <Page size="A4" style={baseStyles.page}>
        ...
        {conversation.citations.length > 0 && (
          <CitationsSection citations={conversation.citations} />
        )}
...
// ============================================================================
// Shared Citations Component
// ============================================================================

function CitationsSection({ citations }: { citations: Citation[] }) {
  return (
    <View style={{ marginTop: 14 }}>
      <Text style={baseStyles.h2}>Citations</Text>
      {citations.map((c: Citation, i: number) => (
        <View key={i} style={{ flexDirection: "row", marginBottom: 2, paddingLeft: 4 }}>
          <Text style={{ fontSize: 9, width: 24, color: colors.subtle }}>[{i + 1}]</Text>
          <Text style={{ fontSize: 9, flex: 1 }}>
            <Text style={baseStyles.bold}>{c.title}</Text>
            <Text style={{ color: colors.subtle }}> ({c.sourceRef})</Text>
          </Text>
        </View>
      ))}
    </View>
...
export async function renderFitReportPdf(
  ...
  const buffer = await renderToBuffer(
    <FitReportPdf analysis={analysis} extracted={extracted} citations={citations} />
  );
...
export async function renderResumePdf(
  ...
  const buffer = await renderToBuffer(
    <ResumePdf content={content} citations={citations} />
  );
  return Buffer.from(buffer);
}
